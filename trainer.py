    ...
    def train(self):
        ...
        def run_epoch(epoch, fake_y):
            ...
            for step, (real_images, real_y) in pbar:
                ...
                real_labels *= config.label_smoothing
            ...
            if epoch == 0:
                save_image(denormalize(real_images[:100]), os.path.join(f"{config.ckpt_path}", "new_images", "real_images.png"), nrow=10)

            save_image(denormalize(fake_images[:100]),
                       os.path.join(f"{config.ckpt_path}", "images", f"fake_images_{epoch + 1}.png"), nrow=10)

            if (epoch + 1) % config.checkpoint_epoch == 0:
                self.save_checkpoint(epoch + 1)

        fake_y = torch.arange(0, 10).repeat(config.batch_size // 10).long().to(self.device)
